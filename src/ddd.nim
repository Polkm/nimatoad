import parsers, opengl, sdl2, camera, entity, model, matrix, vector
import global
...
proc panelsDraw*(): proc() =
  return proc() =
    ...
    for i in low(sTable)..high(sTable):
      ...
      #glRotatef( camera.pitch, 1.0, 0, 0 )
      #glRotatef( camera.yaw, 0, 1.0, 0 )
      glUniformMatrix4fv(glGetUniformLocation(1, "model").int32, 1, false, curS.ent.matrix.m[0].addr)
      glActiveTexture(GL_TEXTURE0)
      glBindTexture(GL_TEXTURE_2D, parseBmp("bmps/rock1.bmp"))
      glActiveTexture(GL_TEXTURE1)
      glBindTexture(GL_TEXTURE_2D, parseBmp("bmps/rock1_normal.bmp"))
      glActiveTexture(GL_TEXTURE0)
      glUniform3f(5, 1.0, 1.0, 1.0)
      glUniform3f(6, 1.0, 1.0, 1.0)
      glUniform3f(8, 1.0, 1.0, 1.0)
      glUniform1f(7, 1.0)
...
proc panelsMouseInput*( button: int, pressed: bool, x,y:float ) =
  ...
  for i in low(sTable)..high(sTable):
    curS = sTable[i]
    #glLoadIdentity()
    #glRotatef( curS.pitch + camera.pitch, 1.0, 0, 0 )
    #glRotatef( curS.yaw + camera.yaw, 0, 1.0, 0 )
    #glRotatef( curS.roll, 0, 0, 1.0 )
    #glTranslatef( curS.xPos, curS.yPos, curS.zPos )
